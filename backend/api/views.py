...
def send_latest_data(request):
    try:
        latest_identification = Identifications.objects.filter(
            Q(classifications__is_solved=False) | Q(classifications__isnull=True)
        ).latest('id')
...
def send_filed_data(request):
    try:
        ...
        for identification in filed_identification:
            try:
                location = filed_location.filter(identification=identification).latest('id')
                image = filed_image.filter(identification=identification).latest('id')
                contact = filed_contact.filter(identification=identification).latest('id')
                classification = filed_classification.filter(identification=identification).latest('id')
...
def send_case_details(request, id):
    ...
    data = {
        ...
        "reported_by": detail_user.first_name +" "+detail_user.last_name+" "+"("+detail_user.username+")"
    }
    return JsonResponse(data)
