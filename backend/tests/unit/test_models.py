...
def test_models_creation():
    ...
    image = Image.objects.create(demographic=demographics, image="test_image.jpg")

    # Create contacts instance
    contacts = Contacts.objects.create(
        demographic=demographics, contact_number="9876543210"
    )

    # Create classification result instance
    classification_result = ClassificationResult.objects.create(
        demographic=demographics, predicted_class="PANIC", is_solved=False
    )
